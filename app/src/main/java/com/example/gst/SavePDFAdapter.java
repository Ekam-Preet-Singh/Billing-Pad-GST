...
public class SavePDFAdapter extends RecyclerView.Adapter<SavePDFAdapter.SavePDFHolder> {
    ...
    @NonNull
    @Override
    public SavePDFHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(@NonNull SavePDFHolder holder, int position) {
        if(mCursor.moveToPosition(position)){

            @SuppressLint("Range") int idValue = mCursor.getInt(mCursor.getColumnIndex(GSTBillingContract.GSTBillingCustomerEntry._ID));
            @SuppressLint("Range") String itemDescriptionValue = mCursor.getString(mCursor.getColumnIndex(GSTBillingContract.GSTBillingCustomerEntry.SECONDARY_COLUMN_ITEM_DESCRIPTION));
            @SuppressLint("Range") float finalPriceValue = mCursor.getFloat(mCursor.getColumnIndex(GSTBillingContract.GSTBillingCustomerEntry.SECONDARY_COLUMN_FINAL_PRICE));
            @SuppressLint("Range") int quantityValue = mCursor.getInt(mCursor.getColumnIndex(GSTBillingContract.GSTBillingCustomerEntry.SECONDARY_COLUMN_QUANTITY));
            @SuppressLint("Range") int taxSlabValue = mCursor.getInt(mCursor.getColumnIndex(GSTBillingContract.GSTBillingCustomerEntry.SECONDARY_COLUMN_TAX_SLAB));
            ...
            holder.taxSlabPdf.setText(String.format("%d%%", taxSlabValue));
    ...
    public static class SavePDFHolder extends RecyclerView.ViewHolder {
